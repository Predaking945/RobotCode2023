...
public class ChargeStationAutoBalance extends CommandBase {
  ...
  private static final LoggedTunableNumber AutoBalanceKP = new LoggedTunableNumber("ChargeStationAutoBalance/AutoBalanceKP");
  private static final LoggedTunableNumber AutoBalanceKI = new LoggedTunableNumber("ChargeStationAutoBalance/AutoBalanceKI");
  private static final LoggedTunableNumber AutoBalanceKD = new LoggedTunableNumber("ChargeStationAutoBalance/AutoBalanceKD");
  double driveSpeedMetersPerSec = 5.0;

  private final PIDController pidController = new PIDController(AutoBalanceKP.get(), AutoBalanceKI.get(), AutoBalanceKD.get());
  ...
  public void execute() {
    ...
    double chargeStationAngle = driveRoll.getRadians() * driveYaw.getSin() + drivePitch.getRadians() * driveYaw.getCos();

    // P controller
    double driveSpeedAfterPID = pidController.calculate(chargeStationAngle);

    // check boundaries for left of charge station
    if (drive.getPose().getX() <= FieldConstants.Community.chargingStationLeftY && driveSpeedAfterPID < 0)
    {
      driveSpeedAfterPID = 0;
    }

    // check boundaries for right of charge station
    if (drive.getPose().getX() <= FieldConstants.Community.chargingStationRightY && driveSpeedAfterPID > 0)
    {
      driveSpeedAfterPID = 0;
    }
